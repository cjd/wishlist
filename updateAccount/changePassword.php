...
function hash(form) {
    ...
    if (valid_js()) {
      ...
      if(form.password.value){
        hash=MD5(form.password.value);
      } else {
        hash="";
        return false;
      }

      form.passwordMD5.value = hash;
      form.password.value = "empty"; // don't send plain text password over net
      form.challenge.value = "empty"; // don't send plain text password over net
    }
...
<?php
  if(isset($_REQUEST['passwordMD5'])){

	$query = "update people set password='" . $_REQUEST['passwordMD5'] . 
        "' where userid='" . $userid . "'";    
...
<center>
<p>
<form name="theForm" method="post" onSubmit="return hash(this);">
